...
$data['numberHorizontal'] = isset($template_param['no_label_hori']) ? (int) $template_param['no_label_hori'] : 3;
$data['numberVertical'] = isset($template_param['no_label_vert']) ? (int) $template_param['no_label_vert'] : 4;
$data['hSpace'] = isset($template_param['hori_label_spacing']) ? (int) $template_param['hori_label_spacing'] : 4;
...
$data['qr_height'] = isset($template_param['qr_code_height']) ? (int) $template_param['qr_code_height'] : 15;
...
//-----------------------------QR-code and NFC AREA---------------------------------------------------------------     
                ...
                if ($this->request->data['Task']['print_type'] == "QR") {
                    ...
                    $pdf->Image($this->Image->generateQRURL($labels[$index]['unique code:']), $pdf->GetX(), $pdf->GetY(), $QR_hw, $QR_hw, 'PNG');
...
//-----------------------------QR-code and NFC AREA---------------------------------------------------------------     
            ...
            if ($data['type'] == "QR") {
                ...
                $QR_hw = ($len_x < $len_y) ? $len_x : $len_y;
                $pdf->Image($this->Image->generateQRURL('290D930028'), $pdf->GetX(), $pdf->GetY(), $QR_hw, $QR_hw, 'PNG');
                $pdf->SetXY($x + $pdf->getProportionalX(6), $pdf->GetY() + $QR_hw + 0.5);
                $pdf->SetTextColor(111, 106, 106);
                $pdf->SetFont('Arial', '', $pdf->font_size);
                $len_x = $pdf->get_remain_x($x, $pdf->GetX(), $QR_hw);
